...
def calculate_difference(reference_face, current_face):
    ...
    return np.mean(cv2.absdiff(reference_face, current_resized))

def detect_face(image):
    face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_frontalface_default.xml")
    return face_cascade.detectMultiScale(image, scaleFactor=1.1, minNeighbors=5, minSize=(50, 50))
...
def verify_with_image():
    """ Endpoint for face verification using base64 images. """
    data = request.json
    ...
    for (x, y, w, h) in webcam_faces:
        ...
        return jsonify({"success": difference < 50, "message": "User Verified" if difference < 50 else "Verification Failed", "difference": float(difference)})
